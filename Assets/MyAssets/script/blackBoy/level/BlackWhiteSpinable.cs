...
public class BlackWhiteSpinable : BlackWhiteRock {
	...
	public EaseType spinEaseType = EaseType.EaseInOutBack;
	...
	public void DoTestSpin(float deltaTime )
	{
		...
		case TestSpinType.SpinLittle:
			...
			HOTween.To( transform , spinTime * spinLittleRate , new TweenParms().Prop("rotation" , to ).Ease(spinEaseType).Loops(2, LoopType.Yoyo));
			break;
	...
	public void Spin()
	{
		...
		HOTween.To( transform , spinTime , new TweenParms().Prop("rotation" , to ).Ease(spinEaseType)
